    ...
    <div className="flex min-h-full bg-paper">
      <aside className="hidden w-[210px] shrink-0 flex-col gap-1.5 bg-ink px-3.5 py-5 md:flex">
        <div className="flex items-center gap-2.5 px-2 pb-3 pt-1">
          ...
          <span className="text-[17px] font-extrabold text-surface">Summit</span>
        </div>
        {navItems.map((item) => (
          <div
            key={item.id}
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="flex h-[58px] shrink-0 items-center justify-between border-b border-hairline bg-surface px-5">
          <h1 className="text-[17px] font-extrabold text-ink">Dashboard</h1>
          <div className="flex items-center gap-3">
            <label className="relative hidden w-[260px] sm:block">
              ...
              <input
                ...
                className="w-full rounded-xl border-[1.5px] border-border-strong bg-field py-2 pl-9 pr-16 text-sm text-ink outline-none placeholder:text-muted focus:border-accent"
              />
